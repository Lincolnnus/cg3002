...
<div id="system-message-container">
   ...
   $s_id=$_SESSION['s_id'];
   else {echo "You are not authorized!";exit;}
   $s_id=$_REQUEST['s_id'];
   $from=$_REQUEST['from'];
   if ($from=='')
   {
	   echo "Please Pick A Start Date!";
	   exit;
   }
   $from=$from.' 00:00:00';
   $till=$_REQUEST['till'];
   if ($till=='')
   {
	   echo "Please Pick A End Date!";
	   exit;
   }
   $till=$till.' 23:59:59';
   if($s_id=='')
   {
       echo "Please Key In Store ID.";
	   exit;
   }
   $res=mysql_query( sprintf("SELECT i.i_id,i.name,t.quantity,t.profit,t.trans_time FROM transaction t, item i WHERE t.i_id=i.i_id and t.s_id=%s and t.trans_time >= '$from' and t.trans_time <='$till' ORDER BY t.trans_time DESC",$s_id));
   if(!$res)
   {
   ...
   if (mysql_num_rows($res))
   {
	   while ($row=mysql_fetch_array($res))
	   {
		   ...
		   echo '<div> Store ID:'.$s_id;
		   echo ' Barcode:'.$row['i_id'];
		   echo ' Item Name:'.$row['name'];
		   echo ' Quantity:'.$row['quantity'];
		   echo ' Profit:$'.$row['profit'].'*'.$row['quantity'].'='.$row['profit']*$row['quantity'].' dollars';
		   echo ' Transaction Date and Time:'.$row['trans_time'].'</div> <br>';
		   $total+=$row['profit']*$row['quantity'];
	   }
	   echo '<div> In Total of '.$total.' dollars</div>';
   }
