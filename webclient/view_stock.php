...
<div id="system-message-container">
   ...
   if ($s_id=='')
   {
		echo "Please Key In the Store ID";
		exit;
   }
   $res=mysql_query("SELECT * FROM stock s,item i WHERE s.i_id=i.i_id and s.s_id=".$s_id);
   if(!$res)
   {
   ...
   if(mysql_num_rows($res))
   {
	   while ($row=mysql_fetch_array($res))
	   {
		   ...
		   echo ' Worth:'.$row['price'].'*'.$row['quantity'].'='.$row['price']*$row['quantity'].' dollars';
		   if ($row['quantity']<$row['stock_floor'])
		   {
			 echo '<font color="red">Please add more stock for this item</font>';
		   }
		   echo '</div> <br>';
