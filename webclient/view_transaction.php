...
<div id="system-message-container">
   ...
   $res=mysql_query( sprintf("SELECT i.i_id,i.name,t.quantity,i.price,t.trans_time FROM transaction t, item i WHERE t.i_id=i.i_id and t.s_id=%s ORDER BY t.trans_time DESC",$s_id));
   if(!$res)
   {
   ...
   if (mysql_num_rows($res))
   {
	   while ($row=mysql_fetch_array($res))
	   {
		   ...
		   echo ' Profit:$'.$row['price'].'*'.$row['quantity'].'='.$row['price']*$row['quantity'].' dollars';
